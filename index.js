...
app.post('/addPlan', (req, res) => {
    ...
    const sql = 'INSERT INTO plan (name, amount, duration) VALUES (?, ?, ?)';
    const values = [name, amount, duration];
    con.query(sql, values, (err, result) => {
      if (err) {
        console.log(err);
        return res.status(500).json({ error: 'An error occurred while creating the enquiry.' });
...
app.put('/updatePlan/:id', (req, res) => {
    const id = req.params.id;
    const { name, amount, duration } = req.body;
    const sql = "UPDATE plan SET name = ?, amount = ?, duration = ? WHERE id = ?";
    const params = [name, amount, duration, id];
    con.query(sql, params, (err, result) => {
      if (err) {
        console.log(err);
...
app.put('/updateMember/:id', (req, res) => {
    const id = req.params.id;
    const { name, planId, contact, email, age, gender, jdate, edate, inamount } = req.body;
    const sql = "UPDATE member SET name = ?, planId = ?, contact = ?, email = ?, age = ?, gender = ?, jdate = ?, edate = ?, inamount = ? WHERE id = ?";
    const params = [name, planId, contact, email, age, gender, jdate, edate, inamount, id];
    con.query(sql, params, (err, result) => {
      if (err) {
        console.log(err);
